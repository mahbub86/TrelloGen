...
export const api = {
  ...
  createBoard: async (title: string, background: string, ownerId: string): Promise<Board> => {
  ...
  shareBoard: async (boardId: string, email: string): Promise<void> => {
  ...
  lookupUserByEmail: async (email: string): Promise<User | null> => {
    const response = await fetch(`${API_BASE_URL}/users/lookup?email=${encodeURIComponent(email)}`);
